...
If you want to build Kemacoin-Qt, make sure that the required packages for Qt development
...
Once these are installed, they will be found by configure and a kemacoin-qt executable will be
built by default.
...
Notes
-----
The release is built with GCC and then "strip kemad" to strip the debug
...
# Pick some path to install BDB to, here we create a directory within the kema directory
BDB_PREFIX="${KEMACOIN_ROOT}/db4"
mkdir -p $BDB_PREFIX
...
# Configure kemacoin Core to use our own-built instance of BDB
cd $KEMACOIN_ROOT
./autogen.sh
...
Security
--------
To help make your kemacoin installation more secure by making certain attacks impossible to
...
* Non-executable Stack
    ...
    vulnerable buffers are found. By default, kemacoin should be built with a non-executable stack
...
Disable-wallet mode
--------------------
When the intention is to run only a P2P node without a wallet, kemacoin may be compiled in
