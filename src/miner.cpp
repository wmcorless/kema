...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
    ...
    // Create new block
    std::unique_ptr<CBlockTemplate> pblocktemplate = std::make_unique<CBlockTemplate>();
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("kemaMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("kemaMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("kemaMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("kema-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        std::unique_ptr<CBlockTemplate> pblocktemplate = std::make_unique<CBlockTemplate>(*CreateNewBlockWithKey(reservekey, pwallet, fProofOfStake));
        if (!pblocktemplate.get())
            continue;
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("kemaMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running kemaMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadkemaMiner() exception");
    } catch (...) {
        LogPrintf("ThreadkemaMiner() exception");
    }

    LogPrintf("ThreadkemaMiner exiting\n");
}
