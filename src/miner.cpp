...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("KETANMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("KETANMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("KETANMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("ketan-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("KETANMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running KETANMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadKETANMiner() exception");
    } catch (...) {
        LogPrintf("ThreadKETANMiner() exception");
    }

    LogPrintf("ThreadKETANMiner exiting\n");
}
